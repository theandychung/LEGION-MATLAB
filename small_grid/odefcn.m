...
for i = u.num_z+1:2:u.num_x*2+u.num_z
    s((i-1)/2) = sum(J_prev{(i-1)/2}.*S(y(i),u.theta_x,u.kappa))...
        -u.W1*S(y(1),u.theta_1,u.kappa)-u.W2*S(y(2),u.theta_2,u.kappa);
%     -u.W1*S(y(1),u.theta_1,u.kappa)
% if i == u.num_z+1
%     sall=[sall s];
% end
    dydt(i) = 3.*y(i)-y(i).^3+2-y(i+1)+u.input((i-1)/2)+s((i-1)/2)+u.rho*randn(1,1);
