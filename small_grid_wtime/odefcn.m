...
% 
temp = fix(t/delay);
if (temp>=size(in,2)*2) % out of map
    out=zeros(size(in));
elseif (temp>=size(in,2))  % exiting map
    temp2 = fix((t-delay*size(in,2))/delay);
    out = [zeros(size(in,1),temp2+1) in(:,size(in,2):-1:temp2+2) ];
else % entering map
    out = [in(:,temp+1:-1:1) zeros(size(in,1),size(in,2)-temp-1)];
end
